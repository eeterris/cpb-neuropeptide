...
feeding_3_raw <- readr::read_csv(
  here("data", "derived", "feeding_3rd.csv")
) %>%
...
write.csv(
  feeding_3_prog,
  here("results", "feeding", "feeding_3_beetle_prog.csv"),
  row.names = FALSE
)
...
write_csv(feeding_3_start_stop, here("results", "feeding", "feeding_3_start_stop.csv"))
...
feeding_4_raw <- readr::read_csv(
  here("data", "derived", "feeding_4th.csv")
) %>%
  select(-`Weight..g.`, -`X..Defoliation`, -Notes, -GroupCore, -Instar, 
...
readr::write_csv(
  feeding_4_prog,
  here("results", "feeding", "feeding_4_beetle_prog.csv")
)

# FIX: Merge corrected C1 developmental data
feeding_4_C1 <- readr::read_csv(
  here("data", "derived", "feeding_4_C1_corrected copy.csv")
)
...
readr::write_csv(
  feeding_4_fixed,
  here("results", "feeding", "feeding_4_beetle_prog_fixed.csv")
)
...
readr::write_csv(
  feeding_4_start_stop,
  here("results", "feeding", "feeding_4_start_stop.csv")
)
...
feeding_4 <- feeding_4_start_stop
